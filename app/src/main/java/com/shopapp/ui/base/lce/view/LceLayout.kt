...
class LceLayout @JvmOverloads constructor(
    ...
    init {
        inflate(context, R.layout.lce, this)
    }

    fun setupContentLayout(@LayoutRes layout: Int) {
        contentView.removeAllViews()
        inflate(context, layout, contentView)
    }
    ...
    sealed class LceState {
        class LoadingState(val isTranslucent: Boolean = false, val useDelay: Boolean = false) : LceState()
