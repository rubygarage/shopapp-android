...
class CategoryRepositoryImpl(private val api: Api) : CategoryRepository {
    ...
    override fun getCategories(perPage: Int, paginationValue: String?,
                               parentCategoryId: String?): Single<List<Category>> {
        return Single.create<List<Category>> { emitter ->
            api.getCategories(perPage, paginationValue, parentCategoryId,
